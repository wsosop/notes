...
public class MyBatisCacheTest {
	 ...
	 * 
	 *和缓存有关的设置/属性：
	 * 			1）、配置文件中的cacheEnabled=true：false：关闭缓存（二级缓存关闭）(一级缓存一直可用的)
	 * 			2）、mapper中每个select标签都有useCache="true"：
	 * 					false：不使用缓存（一级缓存依然使用，二级缓存不使用）
	 * 			3）、mapper中【每个增删改标签的：flushCache="true"：（一级二级都会清除）】
	 * 					增删改执行完成后就会清楚缓存；
	 * 					测试：flushCache="true"：一级缓存就清空了；二级也会被清除；
	 * 					查询标签：flushCache="false"：
	 * 						如果flushCache=true;每次查询之后都会清空缓存（一级二级都会清除）；缓存是没有被使用的；
	...
	*/
	...
	public void testFirstLevelCache() {
		System.out.println("testFirstLevelCache...");
		SqlSession openSession=sqlSessionFactory.openSession();
		try {
