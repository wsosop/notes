...
（其中SLF4j和Log4j还有logback都是出自同一人之手，而Log4j2是借用了Log4j的名称，实则为apache旗下的日志实现框架 ）
...
localhost:8080/abc === 如果没有处理abc,就去静态资源文件夹里面找abc
...
- Inclusion of `ContentNegotiatingViewResolver` and `BeanNameViewResolver` beans.
  ...
  - ==如何定制：我们可以自己给容器中添加一个视图解析器；ContentNegotiatingViewResolver自动的将其组合进来；==
...
<form>
    ...
    <div class="form-group">
        <label>Email</label>
        <input type="email" class="form-control" placeholder="zhangsan@wck.com">
    </div>
    <div class="form-group">
...
​			2）、没有模板引擎（模板引擎找不到这个错误页面），静态资源文件夹（static/error）下找；
...
```shell
1、检查内核版本，必须是3.10及以上，如果需要升级使用 yum update命令 升级
uname -r
2、安装docker
yum install docker
3、输入y确认安装
...
自己用的[root@localhost docker]# docker pull tomcat:8.5.51-jdk8-adoptopenjdk-hotspot 
3、根据镜像启动容器
docker run --name mytomcat -d tomcat:latest
自己用的[root@localhost docker]# docker run --name mytomcat -d tomcat:8.5.40
4、docker ps  
查看运行中的容器
5、 停止运行中的容器
docker stop  容器的id
...
自己用的[root@localhost docker]# docker run --name mytomcat -d tomcat:8.5.40 -p 8080:8080
会报错：/usr/bin/docker-current: Error response from daemon: oci runtime error: container_linux.go:235: starting container process caused "exec: \"-p\": executable file not found in $PATH".
//这个正确了
[root@localhost docker]# docker run -d -p 8888:8080 tomcat:8.5.40
-d：后台运行
...
自己用的[root@localhost ~]# docker run --name mysql01 -e MYSQL_ROOT_PASSWORD=123456 -d mysql:5.5
...
自己用的[root@localhost ~]# docker run -p 3306:3306 --name mysql01 -e MYSQL_ROOT_PASSWORD=123456 -d mysql:5.5
